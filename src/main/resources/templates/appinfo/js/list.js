...
var Render = {
    ...
    info:function(rowdata,renderData){
        var btn=' <button onclick="detailOne(\''+"/appinfo/info"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-mini">详情</button>';
        return btn;
    },
    /**
	 * @param rowdata
    ...
    edit:function(rowdata,renderData){
    	var btn = '';
    	if(hasupdateDepart){
    		btn=' <button onclick="editOne(\''+"/appinfo/edit"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-mini">修改</button>';
    	}
        return btn;
    },
    /**
    ...
    delete:function(rowdata,renderData){
    	var btn = '';
    	if (hasdeleteDepart) {
    		btn='<button onclick="deleteOne(\''+"删除"+'\',\''+"/appinfo/delete"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-mini layui-btn-delete">删除</button>';
		}
        return btn;
    },
    /**
    ...
    state:function(rowdata,renderData){
        if(renderData.state=='0'){
            return' <button onclick="updateStateOne(\''+"下架"+'\',\''+"/appinfo/enable"+'\',\''+rowdata.id+'\')"' +
                '  class="layui-btn layui-btn-mini layui-btn-green">下架</button>';
        }
        if(renderData.state=='1'){
            return' <button  onclick="updateStateOne(\''+"上架"+'\',\''+"/appinfo/limit"+'\',\''+rowdata.id+'\')" ' +
