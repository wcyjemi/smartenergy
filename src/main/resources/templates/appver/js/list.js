...
var Render = {
    ...
    info:function(rowdata,renderData){
        var btn=' <button onclick="detailOne(\''+"/appver/info"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-mini">详情</button>';
        return btn;
    },
    /**
	 * @param rowdata
    ...
    edit:function(rowdata,renderData){
    	var btn = '';
    	if(hasupdateDepart){
    		btn=' <button onclick="editOne(\''+"/appver/edit"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-mini">修改</button>';
    	}
        return btn;
    },
    /**
    ...
    delete:function(rowdata,renderData){
    	var btn = '';
    	if (hasdeleteDepart) {
    		btn='<button onclick="deleteOne(\''+"删除"+'\',\''+"/appver/delete"+'\',\''+rowdata.id+'\')" class="layui-btn layui-btn-mini layui-btn-delete">删除</button>';
		}
        return btn;
    },
    /**
    ...
    state:function(rowdata,renderData){
        if(renderData.state=='0'){
            return' <button onclick="updateStateOne(\''+"下架"+'\',\''+"/appver/enable"+'\',\''+rowdata.id+'\')"' +
                '  class="layui-btn layui-btn-mini layui-btn-green">下架</button>';
        }
        if(renderData.state=='1'){
            return' <button  onclick="updateStateOne(\''+"上架"+'\',\''+"/appver/limit"+'\',\''+rowdata.id+'\')" ' +
                    ...
                    if (result.code == '0') {
                        ...
                        for (var i = 0; i < list.length; i++) {
                            ...
                            imgPart = imgPart + '<div class="float-left"> ' +
                                '<img onclick="imgOpenView(this)" src="' + list[i].img + '" class="layui-upload-img"> ' +
                                '</div>';
                        }
