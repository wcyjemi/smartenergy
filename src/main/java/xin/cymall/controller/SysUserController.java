...
//		//获取所属企业
	...
	public R save(@RequestBody SysUser user){
		...
		if (user.getRoleIdsStr() != null){
			String[] rolearr = user.getRoleIdsStr().split(",");
			for (int i = 0; i<rolearr.length;i++){
				if (StringUtils.isNotBlank(rolearr[i])){
...
//		user.setAppIdList(apps);
	 ...
	 */
	@ResponseBody
	@RequestMapping(value = "/verfiyAdmin")
	public R verfiyAdmin(){
	...
	public void exportUser(HttpServletRequest request, HttpServletResponse response){
		List<SysUser> sysUsers = sysUserService.queryList(new HashMap<String, Object>());
		ExcelUtil.exportExcel(sysUsers,"用户信息","用户信息",SysUser.class,"用户信息.xls",response);
	}
}
